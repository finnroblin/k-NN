...
TEST(FaissIndexUQTest, Load2BitIndexTest) {
    ...
    std::cout << "\n=== DEBUG: Initializing 2-Bit ADC test with seed " << seed << " ===" << std::endl;
    ...
    for (int i = 0; i < numVectors; i++) {
        ...
        for (int d = 0; d < dim; d++) {
            float value = clusterCenters[cluster][d] + distSmall(frng.mt);
    ...
    // Step 2: Quantize vectors to 2-bit unary encoding
    std::cout << "=== DEBUG: Quantizing vectors to 2-bit unary encoding ===" << std::endl;

    // Calculate dimension statistics for thresholds
    std::vector<float> meanValues(dim, 0);
    std::vector<float> stdDevValues(dim, 0);

    // Compute means
    for (int i = 0; i < numVectors; i++) {
        for (int d = 0; d < dim; d++) {
            meanValues[d] += originalVectors[i * dim + d];
        }
    }
    for (int d = 0; d < dim; d++) {
        meanValues[d] /= numVectors;
    }

    // Compute standard deviations
    for (int i = 0; i < numVectors; i++) {
        for (int d = 0; d < dim; d++) {
            float diff = originalVectors[i * dim + d] - meanValues[d];
            stdDevValues[d] += diff * diff;
        }
    }
    for (int d = 0; d < dim; d++) {
        stdDevValues[d] = sqrt(stdDevValues[d] / numVectors);
    }

    // Set thresholds for 2-bit quantization (approximately quartiles for normal distribution)
    std::vector<float> lowThreshold(dim);
    std::vector<float> midThreshold(dim);
    std::vector<float> highThreshold(dim);

    for (int d = 0; d < dim; d++) {
        lowThreshold[d] = meanValues[d] - 0.67f * stdDevValues[d];   // ~25th percentile
        midThreshold[d] = meanValues[d];                             // ~50th percentile
        highThreshold[d] = meanValues[d] + 0.67f * stdDevValues[d];  // ~75th percentile
    }

    // Allocate space for 2-bit encoded vectors - 2 bits per dimension means dim/4 bytes per vector
    std::vector<uint8_t> binaryVectors(numVectors * ((dim * 2 + 7) / 8), 0);

    // Track distribution of quantized values
    std::vector<int> valueCounts(4, 0);

    // Encode using 2-bit unary encoding: 00, 01, 11
    // 00: smallest values (below lowThreshold)
    // 01: medium-small values (between lowThreshold and midThreshold)
    // 11: larger values (above midThreshold)
    // for (int i = 0; i < numVectors; i++) {
    //     for (int d = 0; d < dim; d++) {
    //         float value = originalVectors[i * dim + d];
    //         int bytePos = (i * ((dim * 2 + 7) / 8)) + (d * 2 / 8);
    //         int bitStartPos = (d * 2) % 8;
            
    //         uint8_t code;
    //         if (value < lowThreshold[d]) {
    //             code = 0b00;  // 00
    //             valueCounts[0]++;
    //         } else if (value < midThreshold[d]) {
    //             code = 0b01;  // 01
    //             valueCounts[1]++;
    //         } else if (value < highThreshold[d]) {
    //             code = 0b11;  // 11
    //             valueCounts[2]++;
    //         } else {
    //             code = 0b11;  // 11 (unary encoding doesn't use 10)
    //             valueCounts[3]++;
    //         }
            
    //         // Set the bits in the correct position (2 bits per dimension)
    //         binaryVectors[bytePos] |= (code << bitStartPos);
    //     }
    // }
    // For 2-bit unary encoding with 768 dimensions:
// First 768 bits: first threshold comparison results
// Second 768 bits: second threshold comparison results
    for (int thresholdIdx = 0; thresholdIdx < 2; thresholdIdx++) {  // 2 thresholds for 2-bit encoding
        for (int d = 0; d < dim; d++) {  // dim = 768
            float value = originalVectors[d];
            
            // Calculate bit position within the entire packed representation
            int bitPosition = thresholdIdx * dim + d;
            // Calculate byte index
            int byteIndex = bitPosition >> 3;  // Divide by 8
            // Calculate bit index within the byte (counting from left)
            int bitIndex = 7 - (bitPosition & 7);  // 7 minus (position mod 8)

            if (thresholdIdx == 0) {
                // First threshold comparison
                if (value > midThreshold[d]) {
                    binaryVectors[byteIndex] |= (1 << bitIndex);
                }
            } else {
                // Second threshold comparison
                if (value > lowThreshold[d]) {
                    binaryVectors[byteIndex] |= (1 << bitIndex);
                }
            }
    ...
    // Log statistics about the quantization
    std::cout << "2-bit unary quantization distribution:" << std::endl;
    std::cout << "  00 (smallest): " << (float)valueCounts[0] / (numVectors * dim) * 100 << "%" << std::endl;
    std::cout << "  01 (mid-low): " << (float)valueCounts[1] / (numVectors * dim) * 100 << "%" << std::endl;
    std::cout << "  11 (mid-high): " << (float)valueCounts[2] / (numVectors * dim) * 100 << "%" << std::endl;
    std::cout << "  11 (highest): " << (float)valueCounts[3] / (numVectors * dim) * 100 << "%" << std::endl;

    // Step 3: Calculate ADC statistics for 2-bit quantization
    struct Quantization2BitStats {
        std::vector<float> level0_means;  // Mean of values encoded as 00
        std::vector<float> level1_means;  // Mean of values encoded as 01
        std::vector<float> level2_means;  // Mean of values encoded as 11 (combined for both upper levels)
        
        Quantization2BitStats(int dim) : 
            level0_means(dim, 0), level1_means(dim, 0), level2_means(dim, 0) {}
    };

    Quantization2BitStats stats(dim);
    std::vector<std::vector<int>> levelCounts(3, std::vector<int>(dim, 0));

    // Calculate means for each quantization level
    for (int i = 0; i < numVectors; i++) {
        for (int d = 0; d < dim; d++) {
            float value = originalVectors[i * dim + d];
            
            if (value < lowThreshold[d]) {
                stats.level0_means[d] += value;
                levelCounts[0][d]++;
            } else if (value < midThreshold[d]) {
                stats.level1_means[d] += value;
                levelCounts[1][d]++;
            } else {
                stats.level2_means[d] += value;
                levelCounts[2][d]++;
            }
        }
    }
    ...
    // Normalize the means
    for (int d = 0; d < dim; d++) {
        if (levelCounts[0][d] > 0) stats.level0_means[d] /= levelCounts[0][d];
        if (levelCounts[1][d] > 0) stats.level1_means[d] /= levelCounts[1][d];
        if (levelCounts[2][d] > 0) stats.level2_means[d] /= levelCounts[2][d];
    }

    // Print sample statistics
    for (int d = 0; d < 5; d++) {
        std::cout << "Dim " << d << ": Level0=" << stats.level0_means[d] 
                << ", Level1=" << stats.level1_means[d] 
                << ", Level2=" << stats.level2_means[d] << std::endl;
    }

    // Convert statistics to format needed by FaissIndexBQ
    std::vector<float> below_threshold_means(dim);
    std::vector<float> above_threshold_means(dim);
    for (int d = 0; d < dim; d++) {
        below_threshold_means[d] = stats.level0_means[d];
        above_threshold_means[d] = stats.level2_means[d];
    }

    // Step 4: Create and save binary index (modified for 2-bit)
    std::cout << "=== DEBUG: Creating and saving 2-bit binary HNSW index ===" << std::endl;
    std::string indexPath = test_util::RandomString(10, "tmp/", ".faiss");

    // Use a binary index with appropriate code size for 2-bit quantization
    std::unique_ptr<faiss::IndexBinary> binaryIndex(
        test_util::FaissCreateBinaryIndex(dim * 2, "BHNSW32"));  // Adjusted dimensionality for 2-bit
    ...
    // Step 5: Load binary index and convert to ADC index (now for 2-bit)
    std::cout << "=== DEBUG: Loading 2-bit binary index and converting to ADC index ===" << std::endl;
    ...
    // Create the 2-bit ADC index 
    knn_jni::faiss_wrapper::FaissIndexUQ2Bit* alteredStorage = new knn_jni::faiss_wrapper::FaissIndexUQ2Bit(
        dim, codes, faiss::METRIC_L2, 
        above_threshold_means, below_threshold_means);  // Use UQ index with 2 bits per dim
    ...
    std::cout << "2-bit ADC index created with " << adcIndex->ntotal << " vectors" << std::endl;

    // Step 6: Compute ground truth (unchanged)
    ...
    auto transformedQuery = query;
    auto dc = static_cast<knn_jni::faiss_wrapper::FaissIndexUQ2Bit*>(
...
TEST(FaissIndexUQTest, Basic2BitL2Test) {
    int dim = 8; // so each code should be 2 bytes, each query vector has 8 floats
    faiss::MetricType metric = faiss::METRIC_L2;
    ...
    std::vector<uint8_t> codes = {
        ...
        0b01010101, // 01 01 01 01
        0b11111111, // 11 11 11 11
        0b00010101, // 00 01 01 01
        0b01010100, // 01 01 01 00
        0b11000100  // 11 00 01 00
    }; // 3 vectors in total

    std::vector<float> query = {
        -2, -4, 5, 3.2, 4, -3, 0.1, 0.005
    };
    ...
    // create index 
    knn_jni::faiss_wrapper::FaissIndexUQ2Bit index = knn_jni::faiss_wrapper::FaissIndexUQ2Bit(
        dim, codes, metric, above_threshold_means, below_threshold_means
    );

    faiss::FlatCodesDistanceComputer * computer_ptr = index.get_FlatCodesDistanceComputer();
...
// the distance is defined as p dot z  + correction amount, so we should verify what that is.
    ...
    for (int j = 0; j < distances.size(); ++j) {
        // ASSERT_EQ(ground_truth_distances[j] , distances[j]);
        ASSERT_NEAR(ground_truth_distances[j], distances[j], 1e-4) 
            << "Distance mismatch for vector " << j
            << ": expected " << ground_truth_distances[j] 
            << ", got " << distances[j];
    }
...
TEST(FaissIndexBQTest, InnerProductDistanceComputerTest) {
    ...
    for (int i = 0; i < numVectors; i++) {
        ...
        float distance = dc->distance_to_code(&codes[i * code_size]);
...
TEST(FaissIndexBQTest, Debug2) {
    ...
    knn_jni::faiss_wrapper::FaissIndexBQ* alteredStorage = new knn_jni::faiss_wrapper::FaissIndexBQ(dim, codes, faiss::METRIC_INNER_PRODUCT);
    faiss::IndexHNSW* alteredIndexHNSW = new faiss::IndexHNSW(alteredStorage, hnswM);
    alteredIndexHNSW->hnsw = hnswBinary->hnsw;
    faiss::IndexIDMap* alteredIdMap = new faiss::IndexIDMap(alteredIndexHNSW);
    alteredStorage->init(alteredIndexHNSW, alteredIdMap);
